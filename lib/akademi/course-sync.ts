...
import { fetchShopierProduct, fetchShopierStoreProducts, type ShopierProductDetails } from "../shopier/public-product.ts";
...
async function addDiscoveredCourse(db: Database, productId: string, fetcher: typeof fetch) {
  ...
  const slug = courseSlug(lookup.product.title) || `egitim-${productId}`;
