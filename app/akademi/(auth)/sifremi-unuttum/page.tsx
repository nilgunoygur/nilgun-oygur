...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AuthShell title="Yeni bir başlangıç." description="E-posta adresinizi yazın. Şifrenizi yenilemeniz için bir bağlantı gönderelim."><AuthForm mode="forgot" configured={isAuthConfigured()} token={typeof params.token === "string" ? params.token : undefined} destination={typeof params.next === "string" ? params.next : undefined} initialMessage={message} /></AuthShell>;
}
