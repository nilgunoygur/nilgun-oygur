...
// Rendered on first request, then cached like the catalog page.
export async function generateStaticParams() { return []; }

async function loadCourse(params: Promise<{ slug: string }>) {
  const slug = normalizeSlug((await params).slug);
  return slug ? getCatalogCourse(slug) : null;
}
...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="academy-detail page-width">
    ...
    <div className="academy-detail-grid">
      ...
      <aside className="academy-purchase-card"><p className="academy-kicker">KENDİNİZE BİR ALAN AÇIN</p><h2>Yolculuğunuz burada başlasın.</h2>{demo && <p className="academy-demo-label">Demo eğitim · Örnek fiyat</p>}<strong className="academy-detail-price">{formatPrice(course.priceKurus)}</strong><ul>{[...(course.details ?? []), formatAccess(course.accessDurationDays), "Kendi ritminizde öğrenme", "Kişisel eğitim alanı"].map(x => <li key={x}><Check size={17} aria-hidden="true" />{x}</li>)}</ul>{demo ? <p className="academy-demo-label">Bu bir önizlemedir. Satın alma henüz açık değil.</p> : <Link className={buttonVariants({ size: "hero" })} href={`/akademi/${course.slug}/satin-al`}>Satın al <ArrowUpRight size={18} /></Link>}<Link href="/akademi/giris" className="academy-text-link">Zaten hesabınız var mı? Giriş yapın</Link></aside>
    </div>
  </div>;
}
