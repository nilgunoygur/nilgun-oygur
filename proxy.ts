...
// Real authorization (verified session, owner row, MFA proof) happens in lib/auth/viewer.ts on every request.
export function proxy(request: NextRequest) {
  if (getSessionCookie(request)) return NextResponse.next();
  const { pathname } = request.nextUrl;
  const next = pathname.startsWith("/yonetim/guvenlik") ? "/yonetim/guvenlik" : pathname.startsWith("/yonetim") ? "/yonetim" : "/akademi/hesabim";
