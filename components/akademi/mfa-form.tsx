...
const securityKey = "rounded-[12px] bg-muted p-4 text-[14px] [overflow-wrap:anywhere]";
...
export function MfaForm({ enabled }: { enabled: boolean }) {
  ...
  if (verified) return <div className={formStack}><Alert><AlertDescription>İki aşamalı doğrulama etkinleştirildi. Telefonunuza erişemezseniz aşağıdaki tek kullanımlık kurtarma kodlarını kullanabilirsiniz.</AlertDescription></Alert><p>Kodları güvenli bir yerde saklayın:</p><ul className={securityKey}>{backupCodes.map(code => <li key={code}><code>{code}</code></li>)}</ul><Button size="pill" onClick={() => { setBackupCodes([]); router.replace("/yonetim"); router.refresh(); }}>Kodları kaydettim, devam et</Button></div>;
  return <div className={formStack}>
    {error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
    {secret && <><p>Doğrulayıcı uygulamanızda yeni bir hesap ekleyin. Kurulum anahtarı olarak aşağıdaki kodu, tür olarak zamana dayalı kodu seçin.</p><code className={securityKey}>{secret}</code></>}
