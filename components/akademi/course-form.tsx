"use client";
import { useActionState } from "react";
import { createCourse, type CourseFormState } from "@/app/yonetim/egitimler/actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Checkbox } from "@/components/ui/checkbox";
import { Field, FieldDescription, FieldGroup, FieldLabel } from "@/components/ui/field";
import { Alert, AlertDescription } from "@/components/ui/alert";

const initial: CourseFormState = { status: "idle", message: "" };

export function CourseForm() {
  const [state, action, pending] = useActionState(createCourse, initial);
  return <form action={action}>
    <FieldGroup>
      <Field><FieldLabel htmlFor="title">Eğitim adı</FieldLabel><Input id="title" name="title" required maxLength={120} /></Field>
      <Field><FieldLabel htmlFor="slug">Adres (isteğe bağlı)</FieldLabel><Input id="slug" name="slug" placeholder="dogal-tas-egitimi" pattern="[a-z0-9-]*" /><FieldDescription>Boş bırakırsanız addan oluşturulur: /akademi/…</FieldDescription></Field>
      <Field><FieldLabel htmlFor="description">Açıklama</FieldLabel><Textarea id="description" name="description" rows={4} maxLength={5000} /></Field>
      <div className="grid grid-cols-2 gap-4">
        <Field><FieldLabel htmlFor="price">Fiyat (TL)</FieldLabel><Input id="price" name="price" inputMode="decimal" required placeholder="2490" /></Field>
        <Field><FieldLabel htmlFor="accessDays">Erişim (gün)</FieldLabel><Input id="accessDays" name="accessDays" type="number" min={1} max={3650} defaultValue={365} required /></Field>
      </div>
      <Field><FieldLabel htmlFor="cover">Kapak görseli (isteğe bağlı)</FieldLabel><Input id="cover" name="cover" placeholder="/images/akademi/dogal-tas-v1.png" /></Field>
      <Field><FieldLabel htmlFor="shopierLink">Shopier ürün linki</FieldLabel><Input id="shopierLink" name="shopierLink" placeholder="https://www.shopier.com/51075042" /><FieldDescription>Ürünü Shopier’de oluşturduysanız linkini yapıştırın. Boş bırakırsanız ürün Shopier’de otomatik oluşturulur; fiyatı sonradan Shopier panelinden değiştirirseniz buradan da güncelleyin.</FieldDescription></Field>
      <Field orientation="horizontal"><Checkbox id="hidden" name="hidden" defaultChecked /><FieldLabel htmlFor="hidden">Otomatik oluşturulan ürünü Shopier mağazasında gizle</FieldLabel></Field>
      {state.status !== "idle" && <Alert variant={state.status === "error" ? "destructive" : "default"}><AlertDescription>{state.message}</AlertDescription></Alert>}
      <Button type="submit" disabled={pending}>{pending ? "Kaydediliyor…" : "Eğitimi ekle"}</Button>
    </FieldGroup>
  </form>;
}
