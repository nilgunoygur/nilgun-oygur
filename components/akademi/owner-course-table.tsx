...
import { ownerCatalogQueryOptions, ownerQueryKeys, type OwnerCatalogSnapshot, type OwnerCourse } from "@/lib/akademi/owner-queries";
...
export function OwnerCourseTable({ initialData }: { initialData: OwnerCatalogSnapshot }) {
  ...
  const status = useMutation({
    ...
    onMutate: async ({ id, next }) => {
      ...
      client.setQueryData<OwnerCatalogSnapshot>(queryKey, current => current && ({ ...current, courses: current.courses.map(course => course.id === id ? { ...course, status: next } : course) }));
      return { previous };
    },
