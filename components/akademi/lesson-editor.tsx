...
const button = "inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-forest px-5 py-2 text-sm font-medium text-white hover:bg-forest/90 disabled:opacity-50";
...
export function CourseEditor({ courseId, rows, uploadsEnabled }: { courseId: string; rows: Row[]; uploadsEnabled: boolean }) {
  ...
  return <div className="grid gap-6">
    <form action={action} className="rounded-[24px] bg-mist p-6 sm:p-8"><input type="hidden" name="courseId" value={courseId} /><h2 className="text-2xl">Eğitim programınız</h2><p className="mt-2 mb-6 text-sm leading-relaxed text-stone">Dersleri ekleyin, sıralayın ve hazır olduklarında yayınlayın. Taslakları yalnızca siz görürsünüz.</p><div className="flex flex-wrap gap-3">{rows.length === 0 && <button className={button} name="kind" value="template" disabled={pending}><Plus size={16} />4 video + 1 canlı ders ekle</button>}<button className={button} name="kind" value="video" disabled={pending}><CirclePlay size={16} />Video dersi ekle</button><button className={button} name="kind" value="live" disabled={pending}><CalendarDays size={16} />Canlı ders ekle</button></div><div className="mt-4" role="status"><FormStatus state={state} /></div></form>
...
function EditorCard({ row, uploadsEnabled }: { row: Row; uploadsEnabled: boolean }) {
  ...
  return <details className="group rounded-[22px] border border-border bg-white" open={undefined}>
    ...
    <div className="border-t border-border p-6">
      ...
      <form action={action} className="grid gap-5"><input type="hidden" name="courseId" value={lesson.courseId} /><input type="hidden" name="lessonId" value={lesson.id} />
        ...
        {isLive ? <div className="grid gap-5 rounded-2xl bg-[#fbf6ed] p-5 sm:grid-cols-2"><label className="text-sm font-medium">Başlangıç · İstanbul saati<input className={input} type="datetime-local" name="startsAt" defaultValue={localDate(live?.startsAt)} /></label><label className="text-sm font-medium">Süre (dakika)<input className={input} type="number" name="durationMinutes" min={1} max={1440} defaultValue={live?.durationMinutes ?? 60} required /></label><label className="text-sm font-medium sm:col-span-2">Toplantı bağlantısı<input className={input} type="url" name="joinUrl" placeholder="https://…" defaultValue={live?.zoomJoinUrl ?? ""} maxLength={2048} /></label><label className="text-sm font-medium">Toplantı şifresi (isteğe bağlı)<input className={input} name="passcode" defaultValue={live?.zoomPasscode ?? ""} maxLength={100} /></label><label className="text-sm font-medium">Buluşma durumu<select className={input} name="liveStatus" defaultValue={live?.status ?? "scheduled"}><option value="scheduled">Planlandı</option><option value="rescheduled">Yeniden planlandı</option><option value="cancelled">İptal edildi</option><option value="completed">Tamamlandı</option></select></label></div> : <><input type="hidden" name="startsAt" value="" /><input type="hidden" name="durationMinutes" value="60" /><input type="hidden" name="joinUrl" value="" /><input type="hidden" name="passcode" value="" /><input type="hidden" name="liveStatus" value="scheduled" /></>}
        <div className="flex flex-wrap items-center gap-5"><button className={button} disabled={pending}>{pending ? "Kaydediliyor…" : "Dersi kaydet"}</button><div role="status"><FormStatus state={state} /></div></div>
      </form>
    </div>
  </details>;
}
